...
<div class="row white-bg page-heading">
    ...
    <div class="col-lg-12">
        ...
        <div class="title-action">
			...
			<?php
            $refund_records = \App\Models\CreditNotePayment::where('credit_note_id', '=', $invoice->id)->where('payment_status', '=', 'Success')->count();

            if ( ! in_array( $controller, array( 'InvoiceTasksController', 'InvoiceRemindersController', 'InvoiceNotesController' ) ) ) {
			//$paid_amount = \App\Models\CreditNotePayment::where('credit_note_id', '=', $invoice->id)->where('payment_status', '=', 'Success')->sum('amount');
            $paid_amount = getCache('credit_note_total_paid', 'credit_note_total_paid_' . $invoice->id, 0);
			$total_used = \App\Models\CreditNoteCredit::where('credit_note_id', '=', $invoice->id)->sum('amount');
...
<div class="col-md-6 col-md-offset-6" style="margin-top:50px;">
    <div class="text-right">
        <?php
        $total_paid = \App\Models\CreditNotePayment::where('credit_note_id', $invoice->id)->where('payment_status', 'Success')->sum('amount');
        $total_used = \App\Models\CreditNoteCredit::where('credit_note_id', '=', $invoice->id)->sum('amount');
