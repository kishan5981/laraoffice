...
<div class="form-group">
    ...
    if ( ! empty( $client_project ) ) {
        $billed_tasks = \App\Models\ProjectTask::where('project_id', $client_project->id)->where('billable', 'yes')->where('billed', 'yes')->first();
        $billed_expenses = \App\Models\Expense::where('project_id', $client_project->id)->where('billable', 'yes')->where('billed', 'yes')->first();
