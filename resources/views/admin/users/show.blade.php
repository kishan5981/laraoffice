...
$jdata = $userdata->data($user);
$jsonContent = $jdata->getContent();

$data = json_decode($jsonContent)->data;
    ?>
    @include('support::tickets.partials.datatable', compact('data'))
</div>
@endif
...
@section('javascript') 
    ...
    @endif 
    {{-- @if ( 'active' === $tabs['support_active'] )
        @include('support::tickets.partials.datatable-scripts', [ 'type' => 'agent', 'type_id' => $user->id, 'complete' => false ])
    @endif  --}}
    @if ( 'active' === $tabs['assets_active'] )
        @include('admin.assets.records-display-scripts', [ 'type' => 'contact', 'type_id' => $user->id ])
    @endif
    {{-- @if ( 'active' === $tabs['support_created_active'] )
        @include('support::tickets.partials.datatable-scripts', [ 'type' => 'usersupportcreated', 'type_id' => $user->id, 'complete' => false ])
    @endif --}}
    @if ( 'active' === $tabs['invoices_active'] )
