...
@section('content')
    <h3 class="page-title">  {{ ucfirst($content_page->title) }} </h3>

    <div class="panel panel-default">
        <div class="panel-heading">
           @lang('global.app_view')<span style="float:right;"><i class="fa fa-clock-o"> {{$content_page->created_at->diffForHumans()}}</i></span>
        </div>
                ...
                @php
                  $cols = 8;
                    if ( $content_page->category_id->count() ) {
                        $otherarticles = \App\Models\ContentPage::where('id', '!=', $content_page->id)->whereHas("category_id", function ($query) use( $content_page ) {
