...
{   
        ...
        if (! Gate::allows('language_delete')) {
            ...
            $table->editColumn('language', function ($row) {
                ...
                if ( config('app.action_buttons_hover') ) {
                    $gateKey  = 'language_';
                    $routeKey = 'admin.languages';
                    $language = $language. '<br>' . view('actionsTemplateHover', compact('row', 'gateKey', 'routeKey'));
                }
                return $language;
            });

            $table->rawColumns(['actions','massDelete','language']);
    ...
    public function show($id)
    {
        ...
        $contacts = \App\Models\Contact::whereHas('language',
