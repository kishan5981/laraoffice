    ...
    public function index()
    {
        ...
        if (! Gate::allows('country_delete')) {
            ...
            $table->editColumn('shortcode', function ($row) {
                $shortcode = $row->shortcode ? $row->shortcode : '';
                if ( config('app.action_buttons_hover') ) {
                    $gateKey  = 'country_';
                    $routeKey = 'admin.countries';
                    $shortcode = $shortcode. '<br>' . view('actionsTemplateHover', compact('row', 'gateKey', 'routeKey'));
                }
                return $shortcode;
            });
            ...
            $table->rawColumns(['actions','massDelete', 'shortcode']);
