...
class ContactsController extends Controller
{
    ...
    public function index( $type = '', $type_id = '' )
    {
        ...
        if (request()->ajax()) {
            ...
            */
            ...
            $table->rawColumns(['actions','massDelete','contact_type.title','language.language','contacts.name']);
    ...
    { 
        ...
        if ( ! empty( $type ) && $type == LEADS_TYPE ) {
            $contact_types = \App\Models\ContactType::where('id', LEADS_TYPE)->orderBy('title')->get()->pluck('title', 'id');
        } else {
            $contact_types = \App\Models\ContactType::where('id', '!=', LEADS_TYPE)->orderBy('title')->get()->pluck('title', 'id');
        }
    ...
    public function edit($id)
    {
        ...
        $companies = \App\Models\ContactCompany::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $groups = \App\Models\ContactGroup::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $contact_types = \App\Models\ContactType::orderBy('title')->get()->pluck('title', 'id');
    ...
    public function sendEmail()
    {
        if (request()->ajax()) {
            ...
            if ( $contact ) {
                ...
                \App\Models\ContactNote::create( $contact_notes );
            }
        ...
        if (request()->ajax()) {
            ...
            if ( ! empty( $list_id ) ) {
                foreach($mailchimp_lists['members'] as $list) {
                    $latest_run = \App\Models\ContactEmailCampaigns::orderBy('id', 'desc')->first();
                    if ( ! $latest_run ) {
                        $latest_run = (Object) [
                            'is_schedule' => '',
                            'schedule_date' => '',
    ...
    public function RunMailchimpEmailCampaigns( Request $request, $list_id )
    {
        ...
        if ( $request->isMethod('post') )
        {
            ...
            if ( ! empty( $result['errors'] ) ) {
                ...
                \App\Models\ContactEmailCampaigns::create( $data );
    ...
    public function sendBulkEmailQueue( $contact_type = '' )
    {
        ...
        if (request()->ajax()) {
            $query = \App\Models\ContactType::query();
            ...
            $table->editColumn('member_count', function ($row) use( $contact_type ) {
                return Contact::whereHas("contact_type",
        ...
        $list_deails = '';
        if ( $contact_type ) {
            $list_deails = \App\Models\ContactType::find( $contact_type );
            $template = \Modules\Templates\Entities\Template::where('key', 'bulk-contact-email')->first();

            $emails = Contact::whereHas("contact_type",
    ...
    public function leadConvert( $contact_id, $contact_type_id )
    {
        ...
        $contact_type = \App\Models\ContactType::find( $contact_type_id );
    ...
    public function onMouseHowerActions()
    {
        // Get the current value of the environment variable
        $currentValue = env('ACTION_BUTTONS_HOVER');
        ...
        // Flag to track if the variable was found in .env
        $found = false;
        ...
        foreach ($env as $env_key => $env_value) {
            ...
            if ($entry[0] === 'ACTION_BUTTONS_HOVER') {
                // If yes, overwrite it with the new one
                $env_value = 'ACTION_BUTTONS_HOVER=' . $value;
        ...
        if (!$found) {
            ...
            $env[] = 'ACTION_BUTTONS_HOVER=' . $value;
        }
        ...
        // Return response
        return response()->json(['message' => 'Environment variable updated','status'=>'success']);
    }
}
