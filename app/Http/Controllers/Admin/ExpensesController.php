...
class ExpensesController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            */
            ...
            $table->editColumn('payee.name', function ($row) {                
                return $row->payee ? '<a href="'.route('admin.contacts.show', ['contact_id' => $row->payee->id, 'list' => 'expense']).'">' . $row->payee->name . '</a>' : '-';
            });
    ...
    public function create( $project_id = '' )
    {
        ...
        $expense_categories = \App\Models\ExpenseCategory::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $payees = \App\Models\Contact::whereHas("contact_type",
                   function ($query) {
                       $query->where('id', CUSTOMERS_TYPE);
                   })->get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');

        $payment_methods = \App\Models\PaymentGateway::where('status', '=', 'Active')->get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $projects = \App\Models\ClientProject::all()->pluck('title', 'id')->prepend(trans('global.app_please_select'), '');
        $taxes = \App\Models\Tax::all()->pluck('title', 'id')->prepend(trans('global.app_please_select'), '');
        $project = '';
        if ( ! empty( $project_id ) ) {
            ...
            $currencies = \App\Models\Currency::where('id', $currency_id)->get()->pluck('name', 'id');
            ...
            $currencies = \App\Models\Currency::all()->pluck('name', 'id');
        }
    ...
    public function store(StoreExpensesRequest $request)
    {
        ...
        if ( ! empty( $request->project_id ) ) {
            $project = \App\Models\ClientProject::find( $request->project_id );
            if ( $project ) {
                $currency_id = $project->client->currency_id;
