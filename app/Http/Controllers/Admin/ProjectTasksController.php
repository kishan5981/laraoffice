...
class ProjectTasksController extends Controller
{
    ...
    public function index( $project_id = '' )
    {
        ...
        if (request()->ajax()) {
            ...
            $table->editColumn('name', function ($row) {
                ...
                if ( isAdmin() ) {
                    $timer = \App\Models\TimeEntry::where('task_id', '=', $row->id)->whereNull('end_date')->first();
                } else {
                    $timer = \App\Models\TimeEntry::where('task_id', '=', $row->id)->whereNull('end_date')->where('user_id', Auth::id())->first();
                }
                if ( $timer ) {
    ...
    public function startTimer( $id ) {
        ...
        \App\Models\TimeEntry::create( $data );
    ...
    public function stopTimer( $id, $timer_id ) {
        ...
        $timer = \App\Models\TimeEntry::where('id', '=', $timer_id)->first();
        if ( $timer ) {
    ...
    public function create( $project_id = '' )
    {
        ...
        $recurrings = \App\Models\RecurringPeriod::get()->pluck('title', 'id')->prepend(trans('global.app_please_select'), '');
        $projects = ClientProject::get()->pluck('title', 'id')->prepend(trans('global.app_please_select'), '');
        $mile_stones = \App\Models\MileStone::where('project_id','=',$project->id)->get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $created_bies = \App\Models\User::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
     ...
     *
     * @param  \App\Models\Http\Requests\StoreProjectTasksRequest  $request
     * @return \Illuminate\Http\Response
     */
     ...
     *
     * @param  \App\Models\Http\Requests\UpdateProjectTasksRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\Response
     */
