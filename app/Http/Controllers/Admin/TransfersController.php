...
class TransfersController extends Controller
{
    ...
    public function index( $type = '', $type_id = '' )
    {
        ...
        if (! Gate::allows('transfer_delete')) {
            ...
            */
            ...
            $table->rawColumns(['actions','massDelete','from.name']);
    ...
    public function create()
    {
        ...
        $froms = \App\Models\Account::get()->pluck('namebalance', 'id')->prepend(trans('global.app_please_select'), '');
        $tos = \App\Models\Account::get()->pluck('namebalance', 'id')->prepend(trans('global.app_please_select'), '');
        $payment_methods = \App\Models\PaymentGateway::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
    ...
    public function store(StoreTransfersRequest $request)
    {
        ...
        \App\Models\Account::find($request->from_id)->decrement('initial_balance', $request->amount);
        \App\Models\Account::find( $request->to_id )->increment('initial_balance', $request->amount);
    ...
    public function update(UpdateTransfersRequest $request, $id)
    {
        ...
        \App\Models\Account::find($transfer->from_id)->increment('initial_balance', $transfer->amount);
        \App\Models\Account::find( $transfer->to_id )->decrement('initial_balance', $transfer->amount);
         if ( isDemo() ) {
         return prepareBlockUserMessage( 'info', 'crud_disabled' );
        }
    ...
    public function massDestroy(Request $request)
    {
        ...
        if ($request->input('ids')) {
            ...
            foreach ($entries as $entry) {
                ...
                // Restrore the previous values.
                \App\Models\Account::find($entry->from_id)->increment('initial_balance', $entry->amount);
                \App\Models\Account::find( $entry->to_id )->decrement('initial_balance', $entry->amount);
    ...
    public function restore($id)
    {
        ...
        \App\Models\Account::find($transfer->from_id)->decrement('initial_balance', $transfer->amount);
        \App\Models\Account::find( $transfer->to_id )->increment('initial_balance', $transfer->amount);
