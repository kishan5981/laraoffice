...
use App\Models\ProjectDiscussionComment;
...
class ProjectDiscussionController extends Controller
{
    ...
    public function index( $project_id = '' )
    {
        ...
        if (! Gate::allows('project_discussion_delete')) {
            ...
            $table->editColumn('total_comments', function ($row) {
                $comments = ProjectDiscussionComment::where('project_id', $row->project_id)->where( 'discussion_id', $row->id)->count();
                return $comments;
            });
        ...
        if (request()->ajax()) {
            ...
            $table->editColumn('name', function ($row) {
                ...
                if ( $details && $details->contact_reference_id ) {
                    $contact = \App\Models\Contact::find( $details->contact_reference_id );
                    if ( $contact ) {
    ...
    public function create( $project_id = '' )
    {
        ...
        $projects = \App\Models\ClientProject::get()->pluck('title', 'id')->prepend(trans('global.app_please_select'), '');
     ...
     *
     * @param  \App\Models\Http\Requests\StoreProjectDiscussionsRequest  $request
     * @return \Illuminate\Http\Response
     */
    ...
    public function edit($project_id, $id)
    {
        ...
        return view('admin.project_discussions.edit', compact( 'show_to_customer', 'projects', 'project', 'topic'));
    }

    /**
     * Update ProjectDiscussion in storage.
     *
     * @param  \App\Models\Http\Requests\UpdateProjectDiscussionsRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\Response
     */
