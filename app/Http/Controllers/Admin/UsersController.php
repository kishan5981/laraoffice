...
{   
    ...
    { 
        ...
        if (request()->ajax()) {
            ...
            */
            ...
            $table->rawColumns(['actions','massDelete','role.title','name']);
    ...
    public function create( $contact_id )
    {
        ...
        $contact = \App\Models\Contact::find( $contact_id );
        if ( ! $contact ) {
        ...
        $departments = \App\Models\Department::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
    ...
    public function edit($id)
    {
        ...
        $contact_references = \App\Models\Contact::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $departments = \App\Models\Department::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        ...
        
    {
        ...
        $roles = \App\Models\Role::get()->pluck('title', 'id');

        $user = User::findOrFail($id);
        
        $contact_reference_id = $user->id;

        $contact_references = \App\Models\Contact::get()->pluck('first_name', 'id')->prepend(trans('global.app_please_select'), '');
        $departments = \App\Models\Department::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $user_actions = \App\Models\UserAction::where('user_id', $id)->get();
        $internal_notifications = \App\Models\InternalNotification::whereHas('users',
                    function ($query) use ($id) {
                        $query->where('id', $id);
                    })->get();
        $departments = \App\Models\Department::where('created_by_id', $id)->get();
        $assets_histories = \App\Models\AssetsHistory::where('assigned_user_id', $id)->get();
        $tasks = \App\Models\Task::where('user_id', $id)->get();
        if( isEmployee() ) {
        $client_projects = \App\Models\ClientProject::whereHas('assigned_to',
                    function ($query) use ($id) {
                        $query->where('id', $id);
                    })->get();
        } else {
            $client_projects = \App\Models\ClientProject::where('client_id', '=', $contact_reference_id)->get();
        }

     

        $supports_created = Ticket::where('user_id', $id)->get();
        $assets = \App\Models\Asset::where('assigned_user_id', $id)->get();
    
        $supports = Ticket::where('agent_id', $id)->get();

        $invoices = \App\Models\Invoice::where('customer_id', $contact_reference_id)->get();
    ...
    public function getUser() {
        if (request()->ajax()) {
            ...
            $contact = \App\Models\Contact::find( $contact_reference_id );
