...
class ProjectTicketsController extends Controller
{
    ...
    public function index( $project_id = '' )
    {
        ...
        if (request()->ajax()) {
            ...
            $table->editColumn('agent', function ($row) {
                ...
                //     $name = $row->agent. '<br>' . view('actionsTemplateHover', compact('row', 'gateKey', 'routeKey'));
                // }
                // return $name;
            });
    ...
    protected function PCS()
    {
        ...
        $priorities = Cache::remember('ticketit::priorities', 60, function () {
            return \Modules\Support\Entities\Priority::all();
        });


        $categories = Cache::remember('ticketit::categories', 60, function () {
            return \Modules\Support\Entities\Category::all();
        });


        $statuses = Cache::remember('ticketit::statuses', 60, function () {
            return \Modules\Support\Entities\Status::all();
    ...
    public function edit($project_id, $id)
    {
        ...
        $cat_agents = \Modules\Support\Entities\Category::find($ticket->category_id)->agents()->agentsLists();
