...
class PurchaseOrdersController extends Controller
{
        ...
        if (! Gate::allows('purchase_order_delete')) {
            ...
            */
            ...
            $table->editColumn('invoice_no', function ($row) use($template) {
                    $routeKey = 'admin.purchase_orders';
                $invoice_no = $row->invoice_no ? '<a href="'.route('admin.purchase_orders.show', $row->id).'" title="'.$row->invoice_no.'">' . $row->invoice_no . '</a>' : '';
                if ( ! empty( $row->project_id ) ) {
                    if ( $row->project ) {
                        $title = trans('global.client-projects.invoice-created-from-project', [ 'title' => '<a href="'.route('admin.purchase_orders.show', $row->project_id).'">' . $row->project->title . '</a>'] );
                ...
                if ( config('app.action_buttons_hover') ) {
                    $gateKey  = 'purchase_order_';
                    $routeKey = 'admin.purchase_orders';
                    $invoice_no = $invoice_no . '<br>' . view('actionsTemplateHover', compact('row', 'gateKey', 'routeKey'));
                }
                return $invoice_no;
            });
            ...
            $table->editColumn('amount', function ($row) {
                ...
                $total_paid =  \App\Models\PurchaseOrderPayment::where('purchase_order_id', '=', $row->id)->where('payment_status', 'Success')->sum('amount');
            ...
            $table->rawColumns(['actions','massDelete', 'invoice_no', 'customer.first_name','amount','order']);
    ...
    public function create()
    {
        ...
        $customers = \App\Models\Contact::whereHas("contact_type",
            function ($query) {
        ...
        $warehouses = \App\Models\Warehouse::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
		...
		$enum_discounts_format = \App\Models\Invoice::$enum_discounts_format;
        $enum_tax_format = \App\Models\Invoice::$enum_tax_format;
     ...
     *
     * @param  \App\Models\Http\Requests\UpdatePurchaseOrdersRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\Response
     */
    ...
    public function show($id)
    {
        ...
        return view('admin.purchase_orders.show', compact('invoice'));
    }
    ...
    public function mailInvoice() {
            if (request()->ajax()) {
                ...
                if ( 'sms' === $sub ) {
                    ...
                    PDF::loadView('admin.purchase_orders.invoice.invoice-content', compact('invoice'))->save(  public_path() . '/uploads/purchase_orders/' . $file_name );
                }              
                if ( 'sms' === $sub ) {
                    ...
                    $accounts = \App\Models\Account::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
                    $payment_gateways = \App\Models\Settings::where('moduletype', 'payment')->where('status', '=', 'Active')->get()->pluck('module', 'key')->prepend(trans('global.app_please_select'), '');
                    $categories = \App\Models\ExpenseCategory::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
    ...
    private function insertHistory( $data ) {
        ...
        \App\Models\PurchaseOrderHistory::create( $log );
    }
        ...
        if (request()->ajax()) {
            ...
            if ( isPluginActive('account') ) {
                ...
                if ( ! empty( $post['account'] ) ) {
                    $account = \App\Models\Account::find( $post['account'] );
                    if ( $account ) {
            ...
            $record = \App\Models\PurchaseOrderPayment::create( $data );
            ...
            if ( ! empty( $data['account_id'] ) ) {
                $account_details = \App\Models\Account::find( $data['account_id'] );
            }
            if ( ! $account_details ) {
                $account_id = getSetting('default-account', 'purchase-orders-settings', 0);
                $account_details = \App\Models\Account::find( $account_id ); 
            }
            ...
            if ( ! empty( $account_details ) && 'Yes' === $add_to_expense_po ){
                ...
                $pay_method = \App\Models\PaymentGateway::where('key', '=', $post['paymethod'])->first();
                $pay_method_id = null;
                if ( $pay_method ) {
               ...
               //Let us convert amount to base currency  
                $basecurrency = \App\Models\Currency::where('is_default', 'yes')->first();                
    ...
    public function invoicePDF( $slug, $operation = '') {
        ...
        PDF::loadView('admin.purchase_orders.invoice.invoice-content', compact('invoice'))->save( $path  );
    ...
    public function duplicate( $slug ) {
        ...
        $products_sync = \App\Models\PurchaseOrder::select(['pop.*'])
