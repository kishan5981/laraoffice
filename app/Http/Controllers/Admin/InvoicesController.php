...
class InvoicesController extends Controller
{
    ...
    public function create()
    {
        ...
        $sales_agent = \App\Models\Contact::whereHas("contact_type",
                   function ($query) {
    ...
    private function insertHistory( $data ) {
        ...
        \App\Models\InvoicesHistory::create( $log );
    }
    ...
    public function store(StoreInvoicesRequest $request)
    {
        ...
        if ( ! empty( $request->btnsavesend ) && $customer && 'Published' === $invoice->status ) {
            ...
            $createduser = \App\Models\User::find( $invoice->created_by_id );
            if ( $createduser ) {
    ...
    public function mailInvoice() {
            if (request()->ajax()) {
                ...
                if ( 'sms' === $sub ) {
                    ...
                    PDF::loadView('admin.invoices.invoice.invoice-content', compact('invoice'))->save( public_path() . '/uploads/invoices/' . $file_name );
                }              
                if ( 'sms' === $sub ) {
                    ...
                    $payment_gateways = \App\Models\PaymentGateway::where('status', '=', 'Active')->get()->pluck('name', 'key')->prepend(trans('global.app_please_select'), '');

                    $categories = \App\Models\IncomeCategory::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
    ...
    public function invoiceSend( Request $request ) {
        ...
        if (request()->ajax()) {
            ...
            if ( 'sms' === $sub ) {
                ...
                $res = $this->sendWhatsApp($action, $data);
if (!empty($res['status']) && $res['status'] === 'success') {
    $response['status'] = 'success';
    $response['message'] = 'WhatsApp message generated';
    $response['whatsapp_url'] = $res['url'];
} else {
        ...
        if (request()->ajax()) {
            ...
            if (! empty( $account_details ) && 'yes' === $add_to_income_invoice  ) {
                ...
                if ( $account_details && ! empty( $data['account_id'] ) ) {
                    \App\Models\Account::find($data['account_id'])->increment('initial_balance',$amount);
                }
                ...
                \App\Models\Income::create( $income );
            }
    ...
    public function showPreview( $slug ) {
        ...
        flashMessage( 'danger', 'create', 'Something went wrong');
    }
    }
    ...
    public function invoicePDF( $slug, $operation = 'download') {
        ...
        PDF::loadView('admin.invoices.invoice.invoice-content', compact('invoice'))->save( $path );
    ...
    public function duplicate( $slug ) {
        ...
        $products_sync = Invoice::select(['pop.*'])
    ...
    function sendWhatsApp($action, $data)
{
    ...
    switch ($action) {
        case 'invoice-created':
            ...
            $message .= "👤 *Client:* {$data['client_name']}\n";
            $message .= "📄 *Invoice No:* {$data['invoice_no']}\n";
            $message .= "🔗 *View Invoice:* {$data['invoice_url']}\n\n";
            $message .= "Regards,\n{$data['site_title']}";
            break;

        case 'invoice-reminder':
            $message = "*Invoice Payment Reminder ⚠️*\n\n";
            $message .= "👤 *Client:* {$data['client_name']}\n";
            $message .= "📄 *Invoice No:* {$data['invoice_no']}\n";
            $message .= "💰 *Amount Due:* {$data['amount_due']}\n";
            $message .= "🕒 *Due Date:* {$data['invoice_due_date']}\n";
            $message .= "🔗 *View Invoice:* {$data['invoice_url']}\n\n";
            $message .= "Regards,\n{$data['site_title']}";
            break;

            case 'payment-received':
                $paymentDate = date('d-m-Y'); // You can use 'F j, Y' for a more readable format like April 8, 2025
        ...
        case 'payment-overdue':
            ...
            $message .= "💰 *Amount Due:* {$data['amount_due']}\n";
            $message .= "🔗 *View Invoice:* {$data['invoice_url']}\n\n";
            $message .= "Please clear your dues as soon as possible.\n";
            $message .= "Regards,\n{$data['site_title']}";
            break;
        ...
        default:
            $message = "Hello {$data['client_name']},\nPlease find your invoice here: {$data['invoice_url']}";
            break;
    }

    // Clean phone number
    $phone = preg_replace('/[^0-9]/', '', $data['tonumber']);
    ...
    return [
        'status' => 'success',
        'url' => $whatsAppUrl,
        'message' => 'Formatted WhatsApp message ready.'
    ];
}
