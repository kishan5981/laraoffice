...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    public function sendResetLinkEmail( Request $request, MessageBag $message_bag )
    {
        ...
        $user = \App\Models\User::select('password_resets.token', 'contacts.email')->join('password_resets', 'contacts.email', '=', 'password_resets.email')->where('contacts.email', $request->email)->first();
        //dd( $user );
        if( ! $user ) {
