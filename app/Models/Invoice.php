    ...
    public function history()
{
    ...
    {
        return $this->belongsToMany(\App\Models\PaymentGateway::class, 'invoice_paymentmodes')->withTrashed();
    }

    public function project()
    {
        return $this->belongsTo(\App\Models\ClientProject::class, 'project_id')->withDefault()->withTrashed();
    }
    ...
    {
        if ( $select ) {
            return \App\Models\Invoice::select([
                    'project_tasks.id',
                    'project_tasks.name',
            ...
            return \App\Models\Invoice::select(['pop.*'])
                ->join('invoice_products_tasks as pop', 'pop.invoice_id', '=', 'invoices.id')
                ->join('project_tasks', 'project_tasks.id', '=', 'pop.task_id')
                ->where('invoices.id', $id)->get();
        }
