...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (\Cookie::get('language')) {
            $loc = \Crypt::decrypt(\Cookie::get('language'), false);
            $parts = explode('|', $loc);
        ...
        if( env('SYSTEM_INSTALLED') && env('DB_DATABASE') != '' && ! defined('CRON_JOB') ) {
        ...
        Validator::extend('productinwarehouse', function($attribute, $value, $parameters) {
            ...
            if ( ! empty( $selected_products ) && ! empty( $value ) ) {
                foreach ($selected_products as $product_id) {
                    $product = \App\Models\Product::where('id', $product_id)->where('ware_house_id', $value)->first();
                    if ( ! $product ) {
                        return false;
                    }
                }
