<script>
var controller = '<?php echo e(getController("controller")); ?>';  
...
?>
var currency_id = '<?php echo e($currency_id); ?>';

$('#hdata').data('curr', '<?php echo e($currency_symbol); ?>' );
...
function getProductDetails( id, row ) {
    ...
    if( product_found == 'no') { // Which means product not added to cart.   
        $.ajax({
            url: '<?php echo e(url('admin/search_products')); ?>/' + billtype,
            dataType: "json",
            method: 'post',
        ...
        notifyMe( 'danger', '<?php echo e(trans( "custom.products.already-in-cart-message" )); ?>');
        return false;
    }
}
...
function addproduct() {
    ...
    if ( $('#productselectname-' + cvalue).val() == '') {
        alert('<?php echo e(trans("custom.products.please-select-product")); ?>');
    ...
    if ( tax_type == 'percent' && tax_rate > 100 ) {
        alert('<?php echo e(trans("custom.products.not-more-than-rate")); ?>');
	...
	if ( in_array( $products_selection, array( 'select', 'select2' ) ) ) {
		...
		?>
		productname = '<select class="form-control product_name_select <?php echo $select2; ?>" required="required" name="product_name[]" placeholder="<?php echo e(trans('custom.products.please_select')); ?>" id="productselectname-' + cvalue + '" onchange="getProductDetails(this.value, '+functionNum+')" required><option value=""><?php echo e(trans('custom.products.please_select')); ?></option>';
        <?php if( ! empty( $products_return ) && ! empty( $products_return->project_id ) ): ?>
            productname += '<optgroup label="<?php echo app('translator')->get('global.products.title'); ?>">';
        <?php endif; ?>
		<?php
		if ( ! empty( $products ) ) {
			foreach ($products as $product) {
				?>
				productname+= '<option value="<?php echo $product->id; ?>"><?php echo e($product->name . " (".$product->stock_quantity.")"); ?></option>'
				<?php
			}
        ...
        <?php if( ! empty( $products_return ) && ! empty( $products_return->project_id ) ): ?>
            ...
            if ( $query->count() > 0 ) { ?>
                productname += '<optgroup label="<?php echo app('translator')->get('global.project-tasks.title'); ?>">';
                <?php
                
                foreach ($query->get() as $record ) {
                    ?>
                    productname += '<option value="<?php echo e($record->id); ?>_task"><?php echo e($record->name); ?><?php if( $record->billed == 'yes' ): ?> <?php echo e(trans('global.client-projects.billed')); ?> <?php endif; ?></option>';
                    <?php
                } ?>
                productname += '</optgroup>';
                <?php
            ...
            if ( $query->count() > 0 ) { ?>
                productname += '<optgroup label="<?php echo app('translator')->get('global.expense.title'); ?>">';
                <?php
                ...
                foreach ($query->get() as $record ) {
                    ...
                    ?>
                    productname += '<option value="<?php echo e($record->id); ?>_expense"><?php echo e($record->name . '('.digiCurrency( $record->amount, $products_return->currency_id ).')'); ?><?php if( $record->billed == 'yes' ): ?> <?php echo e(trans('global.client-projects.billed')); ?> <?php endif; ?></option>';
                    <?php
                } ?>
                productname += '</optgroup>'
                <?php
    ...
    <?php if( Gate::allows('product_create') ): ?>
        ...
        productname += '<button type="button" onclick="modalForm(\'createproduct\',' + cvalue + ')" class="btn btn-danger" data-toggle="modal"  data-id="' + cvalue + '" data-post="data-php" data-action="createproduct" data-redirect="<?php echo e(route('admin.purchase_orders.create')); ?>"><?php echo e(trans('global.app_add_new')); ?></button>';
        <?php else: ?>
        productname += '<a onclick="modalForm(\'createproduct\',' + cvalue + ')" class="modalForm" data-toggle="modal"  data-id="' + cvalue + '" data-post="data-php" data-action="createproduct" data-redirect="<?php echo e(route('admin.purchase_orders.create')); ?>"><i class="fa fa-plus-square"></i></button>';
        <?php endif; ?>
    <?php endif; ?>
    ...
    <?php if( 'select2' === $products_selection ): ?>
    ...
    if ( product_id != '' ) {
        ...
        if ( controller != 'PurchaseOrdersController') {
            if ( stock_quantity < quantity ) {
                ...
                alert('<?php echo e(trans("others.products.quantity-not-available")); ?>');
                return false;
           }
        }
...
function quantity_decrease( rowid ) {
    ...
    if ( product_id != '' ) {
        if ( quantity == 1 ) {
            alert('<?php echo e(trans("custom.products.should-be-one")); ?>');
        } else {
            quantity = parseFloat( quantity ) - 1;
            $('#quantity-' + rowid).val( quantity );
            rowTotal( rowid );
...
$('#customer_id').change( function () {
    $.ajax({
        url: '<?php echo e(url('admin/search_products')); ?>/products',
        dataType: "json",
        method: 'post',
...
function getDetails( id, type ) {
	$.ajax({
        url: "<?php echo e(url('admin/get-details')); ?>",
        dataType: "json",
        method: 'post',
...
</script><?php /**PATH C:\xampp\htdocs\laraoffice\resources\views/admin/common/scripts.blade.php ENDPATH**/ ?>