<script src="<?php echo e(asset('adminlte/plugins/ckeditor/adapters/jquery.js')); ?>"></script>
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$('.sendBill').click(function(){
			...
			if ( action == 'make-payment-pay' ) {
				...
				if ( paid_amount >= payable_amount ) {
					loadtemplate = false;
					if( confirm( '<?php echo e(trans("others.invoices.already-paid-proceed")); ?>' ) ) {
						loadtemplate = true;
					}
				}
			}
		...
		$('#mailSend').click(function() {
			...
			var errors = 0;
			if ( toemail == '' ) {
				$('#toemail').after('<span class="error"><?php echo e(trans("custom.invoices.messages.toemail")); ?></a>');
				$('#toemail').focus();
				errors++;
			}
			...
			if ( errors == 0 ) {
				$.ajax({
	                url: '<?php echo e(route("admin.contacts.send-email")); ?>',
	                dataType: "json",
	                method: 'post',
	                data: {
	                	action: action,
	                ...
	                success: function (data) {
	                	$('#loadingModal').modal('toggle');
	                    notifyMe( 'success', '<?php echo e(trans( "custom.messages.mailsent" )); ?>');
	                }
	            });
			}
...
</script><?php /**PATH C:\xampp\htdocs\laraoffice\resources\views/admin/contacts/mail/scripts.blade.php ENDPATH**/ ?>