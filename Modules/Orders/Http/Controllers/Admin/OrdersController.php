...
class OrdersController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            $table->editColumn('id', function ($row) {
                ...
                if ( config('app.action_buttons_hover') ) {
                    $gateKey  = 'order_';
                    $routeKey = 'admin.orders';
                    $id = $id. '<br>' . view('actionsTemplateHover', compact('row', 'gateKey', 'routeKey'));
                }
                return $id;
            });
            ...
            $table->rawColumns(['actions','massDelete', 'id', 'customer.first_name', 'status','id']);
    ...
    public function create()
    {
        ...
        $billing_cycles = \App\Models\RecurringPeriod::get()->pluck('title', 'id')->prepend(trans('orders::global.orders.onetime'), '');
     ...
     *
     * @param  \App\Models\Http\Requests\StoreOrdersRequest  $request
     * @return \Illuminate\Http\Response
     */
    public function store(StoreOrdersRequest $request)
    {
        ...
        $customer_details = \App\Models\Contact::find( $request->customer_id );
        $currency_id = getDefaultCurrency('id');
        ...
        if ( $customer ) {
            ...
            if ( ! empty( $delivery_address_raw['country'] ) ) {
                $country = \App\Models\Country::find( $delivery_address_raw['country'] );
                $country_name = '';
                if ( $country ) {
                ...
                $country = \App\Models\Country::find( $customer->country_id );
                $country_name = '';
                if ( $country ) {
                    $country_name = $country->title;
                }
        ...
        if ( 'Active' === $request->status ) {
            ...
            if ( ! empty( $default_orders_account ) ) {
                $details = \App\Models\Account::find( $default_orders_account );
                if ( ! $details ) {
        ...
        if ( ! empty( $account_details ) && 'yes' === $request->add_to_income ) {
            ...
            if ( $account_details && ! empty( $account_id ) ) {
                ...
                \App\Models\Account::find( $account_id )->increment('initial_balance', $amount);
            }                       
            ...
            $pay_method_id = null;
            if ( $pay_method ) {
                $pay_method_id = \App\Models\PaymentGateway::where('key', '=', $pay_method )->first()->id;
            }
            $income = array(
                'slug' => md5(microtime() . rand()),
     ...
     *
     * @param  \App\Models\Http\Requests\UpdateOrdersRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\Response
     */
    ...
    public function updateCartProduct( Request $request ) {
        if (request()->ajax()) {
            ...
            $product_details = \App\Models\Product::find( $cartorderproduct->product_id );
            if ( $product_details ) {
    ...
    public function payNow( UpdatePaynowRequest $request, $slug, $module ) {
        ...
        $customer = \App\Models\Contact::find( getContactId() );
        $delivery_address = '';
        if ( $customer ) {
            $delivery_address_raw = ( $customer->delivery_address ) ? json_decode( $customer->delivery_address, true ) : array();
        ...
        $customer_id = getContactId();
        $customer_details = \App\Models\Contact::find( $customer_id );
        $currency_id = getDefaultCurrency('id');
        if ( $customer_details && ! empty( $customer_details->currency_id ) ) {
            $currency_id = $customer_details->currency_id;
        }
        ...
        if ( $id > 0 ) {
            $user = \App\Models\Contact::where('id', '=', getContactId())->first();
        ...
        if ( ! empty( $products ) ) {
            ...
            for( $index = 0; $index < count( $names ); $index++ ) {
                ...
                if ( $product->stock_quantity < $product->alert_quantity ) {
                    ...
                    $adminUsers = \App\Models\User::roleadmins()->pluck('contacts.id')->toArray();
                    $notification = array(
                        'text' => trans('orders::global.orders.stock-low') . $names[ $index ],
                        'link' => route('admin.products.edit', $product_id),
                    );
                    $internal_notification = \App\Models\InternalNotification::create( $notification );
    ...
    public function addToIncome( $record, $payment_record ) {
        ...
        $ordercurrency = \App\Models\Currency::find( $currency_id );
        ...
        if ( $account_details && ! empty( $account_id ) ) {
            ...
            \App\Models\Account::find( $account_id )->increment('initial_balance', $converted_amount);
    ...
    public function reOrder( $slug ) {
        ...
        if ( $record->products ) {
            ...
            for( $p = 0; $p < count( $products['product_ids'] ); $p++ ) {               
                ...
                $product = \App\Models\Product::where('id', '=', $product_id )->where('stock_quantity', '>=', $quantity)->first();
                if ( $product ) {
                    $data = array(
                        'quantity' => $quantity,
                        'cart_order_id' => $order->id,
