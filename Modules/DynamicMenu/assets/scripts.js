...
! function(a) {
    ...
    b = wpNavMenu = {
        ...
        initManageLocations: function() {
            // Delegate form submit event handling
            a("#menu-locations-wrap form").on("submit", function() {
                window.onbeforeunload = null;
            });
        
            // Delegate change event handling for menu location select elements
            a(document).on("change", ".menu-location-menus select", function() {
        ...
        attachMenuEditListeners: function() {
            var b = this;
            // Delegate click event handling for menu items' edit, delete, and cancel links
            a("#update-nav-menu").on("click", function(event) {
                if (event.target && event.target.className) {
                    if (-1 != event.target.className.indexOf("item-edit")) return b.eventOnClickEditLink(event.target);
                    if (-1 != event.target.className.indexOf("item-delete")) return b.eventOnClickMenuItemDelete(event.target);
                    if (-1 != event.target.className.indexOf("item-cancel")) return b.eventOnClickCancelLink(event.target);
                }
            });
        
            // Delegate keypress event handling for adding custom links
            a(document).on('keypress', '#add-custom-links input[type="text"]', function(event) {
                if (event.keyCode === 13) {
                    event.preventDefault();
                    a("#submit-customlinkdiv").click();
                }
            });
        },
        ...
        attachQuickSearchListeners: function() {
            var c;
                ...
                a(document).on('keypress', ".quick-search",function(d) {
                var e = a(this);
