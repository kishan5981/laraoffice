...
class ContractsController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            */
            ...
            $table->editColumn('invoice_no', function ($row) use($template) {
                $invoice_no = $row->invoice_no ? '<a href="'.route('admin.contracts.show', $row->id).'" title="'.$row->invoice_no.'">' . $row->invoice_no . '</a>' : '';
                if ( ! empty( $row->project_id ) ) {
                    if ( $row->project ) {
                        $title = trans('global.client-projects.invoice-created-from-project', [ 'title' => '<a href="'.route('admin.contracts.show', $row->project_id).'">' . $row->project->title . '</a>'] );
     ...
     *
     * @param  \App\Models\Http\Requests\StoreContractsRequest  $request
     * @return \Illuminate\Http\Response
     */
     ...
     *
     * @param  \App\Models\Http\Requests\UpdateContractsRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\Response
     */
    ...
    public function mailInvoice() {
            if (request()->ajax()) {
                ...
                if ( 'sms' === $sub ) {
                    ...
                    PDF::loadView('contracts::admin.contracts.invoice.invoice-content', compact('invoice'))->save(  public_path() . '/uploads/contracts/' . $file_name );
                }              
                if ( 'sms' === $sub ) {
    ...
    public function invoicePDF( $slug, $operation = 'download') {
        ...
        PDF::loadView('contracts::admin.contracts.invoice.invoice-content', compact('invoice'))->save( $path  );
