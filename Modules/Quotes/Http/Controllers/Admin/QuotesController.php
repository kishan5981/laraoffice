...
class QuotesController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            */
            ...
            $table->editColumn('invoice_no', function ($row) {
                ...
                $invoice_no = $row->invoice_no ? '<a href="'.route('admin.quotes.show', $row->id).'" title="'.$row->invoice_no.'">' . $row->invoice_no . '</a>' : '';
     ...
     *
     * @param  \App\Models\Http\Requests\StoreQuotesRequest  $request
     * @return \Illuminate\Http\Response
     */
     ...
     *
     * @param  \App\Models\Http\Requests\UpdateQuotesRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\Response
     */
    ...
    public function mailInvoice() {
            if (request()->ajax()) {
                ...
                if ( 'sms' === $sub ) {
                    ...
                    PDF::loadView('quotes::admin.quotes.invoice.invoice-content', compact('invoice'))->save(  public_path() . '/uploads/quotes/' . $file_name );
                }              
                if ( 'sms' === $sub ) {
    ...
    public function invoicePDF( $slug, $operation = 'download') {
        ...
        PDF::loadView('quotes::admin.quotes.invoice.invoice-content', compact('invoice'))->save( $path  );
