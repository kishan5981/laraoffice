...
namespace Modules\Support\Seeds;
...
class TicketitTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($this->statuses as $name => $color) {
            $status = Modules\Support\Entities\Status::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        ...
        foreach ($this->categories as $name => $color) {
            $category = Modules\Support\Entities\Category::create([
                'name'  => $name,
                'color' => $color,
            ]);
        ...
        foreach ($this->priorities as $name => $color) {
            $priority = Modules\Support\Entities\Priority::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        $categories_qty = Modules\Support\Entities\Category::count();
        $priorities_qty = Modules\Support\Entities\Priority::count();
        $statuses_qty = Modules\Support\Entities\Status::count();
        ...
        for ($u = 1; $u <= $this->users_qty; $u++) {
            ...
            for ($t = 1; $t <= $tickets_qty; $t++) {
                ...
                $category = Modules\Support\Entities\Category::find($rand_category);
                ...
                $ticket = new Modules\Support\Entities\Ticket();
                ...
                for ($c = 1; $c <= $comments_qty; $c++) {
                    ...
                    $comment = new Modules\Support\Entities\Comment();
