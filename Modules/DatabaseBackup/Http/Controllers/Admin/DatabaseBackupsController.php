     ...
     public function store(Request $request)
        {
            ...
            if ('database' === $type) {
                $backupconfig['backup.backup.name'] = config('app.name') . '-database';
                config( $backupconfig );
                shell_exec('php artisan backup:run --only-db');
                $backupPath = $backupconfig['backup.backup.name']; // Adjust as per your directory structure
            } elseif ('files' === $type) {
                $backupconfig['backup.backup.name'] = config('app.name') . '-files';
                config( $backupconfig );
                shell_exec('php artisan backup:run --only-files');
                $backupPath = $backupconfig['backup.backup.name']; // Adjust as per your directory structure
            } else {
            ...
            // Save the backup details to the database
            DatabaseBackup::create([
                'name' => $backupconfig['backup.backup.name'],
