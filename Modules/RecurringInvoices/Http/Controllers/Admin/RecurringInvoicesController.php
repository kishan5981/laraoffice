...
class RecurringInvoicesController extends Controller
{
        ...
        if (! Gate::allows('recurring_invoice_delete')) {
            ...
            */
            ...
            $table->editColumn('invoice_no', function ($row) {
                $invoice_no =  $row->invoice_no ? '<a href="'.route('admin.recurring_invoices.show', $row->id).'" title="'.$row->invoice_no.'">' . $row->invoice_no . '</a>' : '';
     ...
     *
     * @param  \App\Models\Http\Requests\StoreRecurringInvoicesRequest  $request
     * @return \Illuminate\Http\Response
     */
     ...
     *
     * @param  \App\Models\Http\Requests\UpdateRecurringInvoicesRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\Response
     */
    ...
    public function mailInvoice() {
            if (request()->ajax()) {
                ...
                if ( 'sms' === $sub ) {
                    ...
                    PDF::loadView('recurringinvoices::admin.recurring_invoices.invoice.invoice-content', compact('invoice'))->save(  public_path() . '/uploads/recurring_invoices/' . $file_name );
                }              
                if ( 'sms' === $sub ) {
    ...
    public function invoicePDF( $slug, $operation = 'download') {
        ...
        PDF::loadView('recurringinvoices::admin.recurring_invoices.invoice.invoice-content', compact('invoice'))->save( $path );
    ...
    public function duplicate( $slug ) {
        ...
        $products_sync = \App\Models\Invoice::select(['pop.*'])
	...
	public function getDetails( $id ) {
		if (request()->ajax()) {
			return json_encode( \App\Models\RecurringPeriod::find( $id ) );
        }
	}
}
