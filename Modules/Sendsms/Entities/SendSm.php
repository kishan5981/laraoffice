...
class SendSm extends Model
{
    ...
    public static function sendSms($data) 
    {
        ...
        if (empty($text) && !empty($data['invoice_id'])) {
            $invoice = \App\Invoice::with('customer')->find($data['invoice_id']);
            if ($invoice && $invoice->customer) {
                $customerName = $invoice->customer->first_name . ' ' . $invoice->customer->last_name;
                $invoiceNo = $invoice->invoice_no ?? $invoice->invoicenumberdisplay;
                $billDate = optional($invoice->created_at)->format('d-m-Y');
                $dueDate = optional($invoice->order_due_date)->format('d-m-Y');
                $amountDue = digiCurrency($invoice->amount, $invoice->currency_id);
                $invoiceLink = route('admin.quotes.preview', ['slug' => $invoice->slug]);
                ...
                $text = "Invoice details not found.";
            }
        }
