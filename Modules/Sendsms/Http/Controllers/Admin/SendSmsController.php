...
{   
    ...
    public function index()
    {
        ...
        if (! Gate::allows('send_sm_delete')) {
            ...
            $table->editColumn('send_to', function ($row) {
                {
                    $send_to= $row->send_to ? $row->send_to : '';
                    if ( config('app.action_buttons_hover') ) {
                        $gateKey  = 'send_sms_';
                        $routeKey = 'admin.send_sms';
                        $send_to = $send_to. '<br>' . view('actionsTemplateHover', compact('row', 'gateKey', 'routeKey'));
                    }
                    return $send_to;
                }
            });
            ...
            $table->rawColumns(['actions','massDelete','send_to']);
    ...
    public function create()
    {
        ...
        $clients = \App\Models\Contact::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
     ...
     *
     * @param  \App\Models\Http\Requests\StoreSendSmsRequest  $request
     * @return \Illuminate\Http\Response
     */
    ...
    public function store(StoreSendSmsRequest $request)
    {
        ...
        // ✅ 2. Default gateway is now WhatsApp, no need for DB check unless used for logging
        $default_sms_gateway = 'whatsapp';
    
        // You could remove this check since you hardcoded the gateway
        if (empty($default_sms_gateway)) {
            flashMessage('danger', 'create', 'sendsms::global.send-sms.no-gateway');
            return back();
        }
    
        // ✅ 3. Prepare WhatsApp message data
        $data['tonumber']  = $request->send_to;
        $data['content']   = $request->message;
    
        // Optionally, enrich with additional data like due amount, invoice number, etc.
        // $data['invoice_no'] = 'INV-1001'; $data['amount'] = '₹3,000'; etc.
    
        // ✅ 4. Call WhatsApp message sender (reused SMS function)
        $res = SendSm::sendSms($data);
    
        // ✅ 5. Error handling
        if (!empty($res['status']) && $res['status'] === 'failed') {
            flashMessage('danger', 'create', $res['message']);
            return back();
        }
    
        // ✅ 6. Log the attempt (optional)
        $gateway = \Modules\Sendsms\Entities\SmsGateway::where('key', $default_sms_gateway)
                    ->withTrashed()
                    ->first();
        ...
        // ✅ 7. Handle demo mode (optional)
        if (isDemo()) {
            return prepareBlockUserMessage('info', 'crud_disabled');
        }
    
        // ✅ 8. Create the log record
        $send_sm = SendSm::create($request->all());
    
        flashMessage($res['status'], 'create', $res['message']);
    
        // ✅ 9. Redirect user directly to WhatsApp chat
        return redirect()->away($res['whatsapp_url']);
    }
     ...
     *
     * @param  \App\Models\Http\Requests\UpdateSendSmsRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\Response
     */
