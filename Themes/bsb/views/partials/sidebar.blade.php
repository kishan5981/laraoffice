...
<section>
    ...
    <aside id="leftsidebar" class="sidebar">
        ...
        <!-- User Info -->
        <?php
        $contact = \App\Models\Contact::where( 'id', '=', getContactId() )->first();
        if ( $contact ) {
        ...
        <div class="menu">
            ...
            @if( isAdmin())
                ...
                @if( isPluginActive( ['invoice', 'credit_note', 'quotes'] ) )
                    ...
                    <li class="{{ ( in_array($controller, array('InvoicesController', 'QuotesController') ) && in_array($action, array('index', 'create', 'edit', 'show', 'destroy', 'uploadDocuments') ) ) ? 'active' : '' }}">
                        ...
                        <ul class="ml-menu">
                            ...
                            @if( File::exists(config('modules.paths.modules') . '/Quotes') && Module::find('quotes') && isPluginActive('quotes'))
                                @can('quote_access')
                            ...
                            @if( File::exists(config('modules.paths.modules') . '/Proposals') && Module::find('proposals') && isPluginActive('proposals'))
                                @can('proposal_access')
                                <li>
                            ...
                            @if( File::exists(config('modules.paths.modules') . '/Contracts') && Module::find('contracts') && isPluginActive('contracts'))
                                @can('contract_access')
                                <li>
                ...
                @if( File::exists(config('modules.paths.modules') . '/RecurringInvoices') && Module::find('recurringinvoices') && isPluginActive('recurringinvoices'))
        ...
        <li class="header">@lang('custom.menu.miscellaneous')</li>
            ...
            <li class="{{ ( in_array( $request->segment(1), array( 'internal_notifications' ) ) || ( in_array($controller, array('SendSmsController') ) && in_array($action, array('index', 'create', 'edit', 'show', 'destroy') ) ) ) ? 'active' : '' }}">
                ...
                <ul class="ml-menu">
                    ...
                    @if( File::exists(config('modules.paths.modules') . '/Sendsms') && Module::find('sendsms') && isPluginActive('Sendsms'))
                        @can('send_sm_access')
         ...
         <!-- knowledgebase_access -->
            ...
            <li class="{{ ( in_array( $request->segment(1), array( 'support' ) ) || in_array( $request->segment(2), array(  'articles', 'faq_questions', 'faq_categories' ) ) ) ? 'active' : '' }}">
                <a href="#" class="menu-toggle">
            ...
            @if ($unread = App\Models\MessengerTopic::countUnread())
